...
void init()
{
	...
	GLushort * indices = new GLushort[np*2];
	int o = 0;
	int q = 0;
	int p = 0;
	for (int k = 0; k < totalSamples.size(); k++)
	{
		//int np = totalSamples[k].size();
		int npp = totalSamples[k].size();
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, np * 2 * sizeof(GLushort), indices, GL_STATIC_DRAW);
...
void displayFunc() {
    ...
    view = glm::lookAt(glm::vec3(eyex+cx, eyey+cy, eyez+cz),
	...
	{
		glDrawElements(GL_LINES, n_lines[i], GL_UNSIGNED_SHORT, (GLvoid*)(countLines * sizeof(GLushort)));
		countLines += n_lines[i];
	}
	/*for (int i = 0; i < indices.size(); i++)
	{
		glDrawElemets(GL_LINE_STRIP, 1, GL_UNSIGNED_SHORT, (GLvoid*)())
	}*/
