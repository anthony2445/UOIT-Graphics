...
void init() {
	...
	*/
	...
	for (int i = 0; i < model->nface; ++i) {
		...
		//allow normals to equal 3 face values to produce shaded triangles based on model coordinates
	}
	...
	*/
	...
	glVertexAttribPointer(vNormal, 3, GL_FLOAT, GL_FALSE, 0, (void*) (sizeof(vertices)* sizeof(GLfloat)));
...
void displayFunc() {
	...
	glUniformMatrix4fv(viewLoc, 1, 0, glm::value_ptr(modelViewPerspective));
	...
	glDrawElements(GL_TRIANGLES, 9 * triangles, GL_UNSIGNED_SHORT, NULL);
	glutSwapBuffers();
}
