...
void init() {
	 ...
	 */
	sprintf(vname,"phong.vs", vertexName);
	sprintf(fname,"phong.fs", fragmentName);
...
void displayFunc() {
	...
	int lightLoc;
	int colour;
	lx = cx + 250.0*cos(angle);
	ly = cy;
	lz = cz + 250.0*sin(angle);
